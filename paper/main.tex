...
% ---- Paper metadata ----
\title{Nothing to Forget: Cross-Lingual Unlearning Evaluation\\
Needs a Transfer Control}
...
English-only fine-tuning never implanted the facts in the other three
varieties at all — it raised forget-set loss there, and raised retain-set
loss by an indistinguishable amount — so there was no cross-lingual memory
to leak.
...
problem with how the measurement is made.
...
A fact that was never acquired in Arabic cannot leak in Arabic, and an
evaluation that omits the base model cannot tell the two situations apart.
...
predictable in variety $v$ at all. If $\Delta_v^{\text{tr}} \geq 0$, the
anchor never held the fact in $v$, and a small $\Delta_v^{\text{unl}}$
carries no information about forgetting: there is nothing there to remove.
Reporting leakage in that regime is a false positive.
...
facts the transfer control says were never acquired. Hindi is
...
requirements follow. Report the base model per language, not once overall.
Report a retain-set probe alongside every forget-set probe, so that
fact-specific movement can be separated from drift. And state the transfer
delta as a precondition of the leakage claim: where $\Delta_v^{\text{tr}}$
does not show acquisition in variety $v$, no statement about forgetting in
$v$ is licensed. The cost is inference only, on adapters the pipeline
already produces.
...
loss by a statistically indistinguishable amount, so no fact-specific
knowledge was ever transferred for unlearning to leave behind. Net of both
controls the MSA effect is $+0.034$ nats, $3.5\%$ of English, on facts the
model never knew in Arabic.
